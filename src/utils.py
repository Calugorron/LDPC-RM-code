...
def show_group(matrix_list, rows):
    try:
        height = int(np.ceil(len(matrix_list)/rows))
        _, axs = plt.subplots(height, rows, layout='constrained')
        for idx, matrix in enumerate(matrix_list):
            axs[idx//rows,idx%rows].set_aspect('equal')
            #print(row)
            axs[idx//rows,idx%rows].imshow(matrix, cmap='Greys')
    except ValueError:
        print("The matrix list is empty")

def show_heatmap(matrix_list):
    heatmap = np.zeros_like(matrix_list[0])
    
    for matrix in matrix_list:
        np_matrix = np.asarray(matrix)
        heatmap = heatmap + np_matrix
